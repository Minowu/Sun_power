...
const Products = () => {
  ...
  const { data: items, loading, error } = useProducts();
  ...
  const categories = useMemo<Category[]>(() => {
    ...
    items.forEach(item => {
      if (!catMap.has(item.category)) {
        catMap.set(item.category, new Set(['all']));
      }
      catMap.get(item.category)?.add(item.subCategory);
    });
    ...
    return Array.from(catMap.entries())
      ...
      .sort((a, b) => a.id.localeCompare(b.id)); // Sort categories for consistent order
  }, [items]);

  // Set default category when items load
  useEffect(() => {
    if (categories.length > 0 && !selectedCategory) {
      const firstCategory = categories[0];
      setSelectedCategory(firstCategory.id);
      const firstSubCategory = firstCategory.subCategories.find(sub => sub !== 'all');
      if (firstSubCategory) {
  ...
  useEffect(() => {
    ...
    const firstSubCategory = category?.subCategories.find(sub => sub !== 'all');
    if (firstSubCategory) {
      setSelectedSubCategory(firstSubCategory);
    }
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-4 gap-2 mt-2 max-md:grid-cols-1 max-md:gap-4 max-md:!grid-rows-[auto_auto]" style={{gridTemplateRows: 'auto 1000px'}}>
        ...
        <div className="row-start-2 grid max-md:col-span-1">
          ...
          <div className="col-span-3 row-start-2 grid grid-cols-3 gap-4 max-md:col-span-1 max-md:grid-cols-1 max-md:gap-2">
              {loading && (
                <div className="col-span-3 text-center py-12 text-gray-500">{t('messages.loading')}</div>
              )}
              {error && (
